...
bool exclLepto(const PaEvent &e) {
	...
	return is_pi0; 	// Return true if this is a pi0-exclusive event
}
...
// *************************  CLEAR GEN VALS (DVCS)  ***************************
// This funtion is used to reset values per event to prevent stale values from being saved to the tree  
// This may not be necessary depending on the structure of the user event 
// 1. Function: buildClusterVecs() -> build the TLorentz vectors for the clusters 
// 2. Input (14): 13 doubles and 1 boolean (exact details are below)
// 3. Output (1): nothing -> it will jsut modify the already existing values 
// 4. Example usage: https://github.com/gursimrankainth/git_COMPASS/blob/main/u970_DVCS.cc 
void resetGenVars(
  double &y_gen,
  double &Q2_gen,
