...
bool pi0_selec = false; // Set true for exclusive pi0 selection for LEPTO Pi0 
...
void UserEvent970(PaEvent & e) { // begin event loop
  ...
  static TVector3 outMu_vec;    // position vector for the scattered muon (use in pull calculations)
  ...
  static double t_fit;  
  static double xbj_fit;
  static double phi_gg_fit;  

  //*******************************************
  // HEPGEN BH data (prior to event selection) -> used for acceptance study 
  static double phi_gg; // azimuthal angle between the virtual and real photon production planes 
  static double phi_gg_gen;
  ...
  static double weight_PAMBH_gen;

  static TLorentzVector inMuGen_TL; 
  static TLorentzVector outMuGen_TL; 
  static TLorentzVector gammaGen_TL; 
  static TLorentzVector protonGen_TL;  
  ...
  if (first) { // histograms and Ntuples booking block
      ...
      tree_gen->Branch("weight_PAMBH_gen", &weight_PAMBH_gen, "weight_PAMBH_gen/D");
  ...
  for (int iv = 0; iv < e.NVertex(); iv++) { // Begin loop over vertices
    ...
    if (!genFilled && e.IsMC() && !leptoMC) { // Begin loop over MC data 
      for (int iv = 0; iv < e.NMCvertex(); iv++) { 
        ...
        int it_beam = v.iBeam(); 
        if (v.NMCtrack() != 4) continue; 

        const PaMCtrack &t_beam   = e.vMCtrack(v.iMCtrack(0));
        const PaMCtrack &t_outMu  = e.vMCtrack(v.iMCtrack(1));
        ...
        const PaTPar &par_outMu  = t_outMu.ParInVtx();
        const PaTPar &par_gamma  = t_gamma.ParInVtx();
        const PaTPar &par_proton = t_proton.ParInVtx();

        inMuGen_TL   = par_beam.LzVec(M_mu);
        outMuGen_TL  = par_outMu.LzVec(M_mu);
        gammaGen_TL  = par_gamma.LzVec(0);
        protonGen_TL = par_proton.LzVec(M_p);
        ...
        weight_PAMBH_gen = Weight_Pam_BH(xbj_gen, Q2_gen, phi_gg_gen, t_gen, E_beam_gen, phase_fac); 
      ...
      for (auto proton: proton_candidates) { // Begin loop over proton candidates
        ...
        for (auto iLow = std::size_t{0}; iLow < pi0_cl_id.size(); ++iLow) {  
          ...
          if (protonPassCount >= 4) {
            eventFlags.setFlagByName("nPi0Save_flag", true);
            tree->Fill();
          }
