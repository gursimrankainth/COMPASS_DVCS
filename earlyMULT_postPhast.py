...
input_path = "/eos/user/g/gkainth/BH/mergedFiles/P04_muPlus/"
...
output_file = f"earlyMULT_filtered_P{period}.root"
...
for event_key, event_candidates in events.items(): # loop over the events 
  ...
  # apply kinematic cuts - choose representative candidate for check, if failed skip the event 
  candidate = event_candidates[0]
  chain.GetEntry(candidate.idx)
  event = chain # alias for clarity 

  #print("test: ", event_key, candidate.combo_key)
  #print("test: ", event.Q2_fit, event.y_fit, event.t_fit, event.nu_fit)
