...
def get_S(real_sum: np.ndarray, BH_sum: np.ndarray, lepPi0_sum: np.ndarray, hepPi0_sum: np.ndarray,
          period_idx: int, charge: str = "muPlus") -> np.ndarray:
  if charge == "muPlus":
    sum_term = real_sum - const.CBH_MUPLUS[period_idx]*BH_sum - const.CPI0_LEP_MUPLUS[period_idx]*const.R_LEPTO*lepPi0_sum - const.CPI0_HEP_MUPLUS[period_idx]*(1-const.R_LEPTO)*hepPi0_sum
  elif charge == "muMinus": 
    sum_term = real_sum - const.CBH_MUMINUS[period_idx]*BH_sum - const.CPI0_LEP_MUMINUS[period_idx]*const.R_LEPTO*lepPi0_sum - const.CPI0_HEP_MUMINUS[period_idx]*(1-const.R_LEPTO)*hepPi0_sum
  else:
...
def get_var_S(D_array: np.ndarray, B_array: np.ndarray, L_array: np.ndarray, H_array: np.ndarray,
  ...
  t1 = D_array
  if charge == "muPlus":
    t2 = (const.CBH_MUPLUS[period_idx])**2 * B_array
    t3 = (const.CPI0_LEP_MUPLUS[period_idx])**2 * (const.R_LEPTO)**2 * L_array
    t4 = (const.CPI0_HEP_MUPLUS[period_idx])**2 * (1-const.R_LEPTO)**2 * H_array
    sum_term = t1 + t2 + t3 + t4
  elif charge == "muMinus": 
    t2 = (const.CBH_MUMINUS[period_idx])**2 * B_array
    t3 = (const.CPI0_LEP_MUMINUS[period_idx])**2 * (const.R_LEPTO)**2 * L_array
    t4 = (const.CPI0_HEP_MUMINUS[period_idx])**2 * (1-const.R_LEPTO)**2 * H_array
    sum_term = t1 + t2 + t3 + t4
  else:
...
def main() -> None:
  ...
  debug_log = None
  if debug_main:
    debug_log = open("dvcs_xSection_debug.log", "w")
    debug_print = lambda *args, **kwargs: print(*args, file=debug_log, **kwargs)
    print("DEBUG MODE ACTIVE: Check dvcs_xSection_debug.log and G_arrays_P0X.pkl for additional info.")
  else:
    debug_print = print
  ...
  for idx, period in enumerate(const.PERIODS[:1]):
    print(idx, period)
    # ***********************************************
    # *          *** 4D ACCEPTANCE ***              * 
    # ***********************************************
    ...
    if debug_main: 
      debug_print(f"Total real μ+ events: {np.sum(real_ijkl_muPlus)}")
      debug_print(f"Total real μ- events: {np.sum(real_ijkl_muMinus)}")
    ...
    if debug_main:
      debug_print(f"Total BH μ+ events: {np.sum(BH_ijkl_muPlus)}")
      debug_print(f"Total BH μ- events: {np.sum(BH_ijkl_muMinus)}")
    ...
    if debug_main:
      debug_print(f"Total inc. pi0 μ+ events: {np.sum(lepPi0_ijkl_muPlus)}")
      debug_print(f"Total inc. pi0 μ- events: {np.sum(lepPi0_ijkl_muMinus)}")
    ...
    if debug_main:
      debug_print(f"Total exc. pi0 μ+ events: {np.sum(hepPi0_ijkl_muPlus)}")
    ...
    if debug_main: 
      ...
      debug_print("mu- varD min/max:", np.min(varD_ijkl_muMinus), np.max(varD_ijkl_muMinus))
    
    if debug_main:
      debug_print("mu+ varB min/max:", np.min(varB_ijkl_muPlus), np.max(varB_ijkl_muPlus))
      debug_print("mu- varB min/max:", np.min(varB_ijkl_muMinus), np.max(varB_ijkl_muMinus))

    if debug_main:
      debug_print("mu+ varL min/max:", np.min(varL_ijkl_muPlus), np.max(varL_ijkl_muPlus))
      debug_print("mu- varL min/max:", np.min(varL_ijkl_muMinus), np.max(varL_ijkl_muMinus))

    if debug_main:
      debug_print("mu+ varH min/max:", np.min(varH_ijkl_muPlus), np.max(varH_ijkl_muPlus))
      debug_print("mu- varH min/max:", np.min(varH_ijkl_muMinus), np.max(varH_ijkl_muMinus))
    ...
    if debug_main:
      debug_print("mu+ varS min/max:", np.min(varS_ijkl_muPlus), np.max(varS_ijkl_muPlus))
      debug_print("mu- varS min/max:", np.min(varS_ijkl_muMinus), np.max(varS_ijkl_muMinus))

    if debug_main: # detailed error calculation check 
      debug_print("Error diagnostics (muPlus only):")
      debug_print("S_ijkl: min =", np.min(sum_ijkl_muPlus), "max =", np.max(sum_ijkl_muPlus))
      debug_print("A_ijkl: min =", np.min(acc_muPlus), "max =", np.max(acc_muPlus))
      debug_print("varS_ijkl: min =", np.min(varS_ijkl_muPlus), "max =", np.max(varS_ijkl_muPlus))
      debug_print("varA_ijkl: min =", np.min(varAcc_muPlus), "max =", np.max(varAcc_muPlus))
      debug_print("S/A ratio: min =", np.min(sum_ijkl_muPlus/acc_muPlus), "max =", np.max(sum_ijkl_muPlus/acc_muPlus))
      debug_print("varS/S^2: min =", np.min(varS_ijkl_muPlus/(sum_ijkl_muPlus**2)), "max =", np.max(varS_ijkl_muPlus/(sum_ijkl_muPlus**2)))
      debug_print("varA/A^2: min =", np.min(varAcc_muPlus/(acc_muPlus**2)), "max =", np.max(varAcc_muPlus/(acc_muPlus**2)))
      debug_print("(S/A)^2 ratio: min =", np.min((sum_ijkl_muPlus**2)/(acc_muPlus**2)), "max =", np.max((sum_ijkl_muPlus**2)/(acc_muPlus**2)))
