...
# TODO: adjust the axis titles and labels so they are larger
# TODO: re-make the default plots with the tighter final binning as well 
...
rec_files = [
  "/eos/user/g/gkainth/BH_filtered/filtered_P04_muPlus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_P04_muMinus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_P05_muPlus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_P05_muMinus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_P06_muPlus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_P06_muMinus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_P07_muPlus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_P07_muMinus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_P08_muPlus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_P08_muMinus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_P09_muPlus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_P09_muMinus.root"
]

tree_rec = ROOT.TChain("USR970_filtered")
for f in rec_files:
  tree_rec.Add(f)

# Generated MC data 
gen_files = [
  "/eos/user/g/gkainth/BH_filtered/filtered_gen_P04_muPlus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_gen_P04_muMinus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_gen_P05_muPlus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_gen_P05_muMinus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_gen_P06_muPlus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_gen_P06_muMinus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_gen_P07_muPlus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_gen_P07_muMinus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_gen_P08_muPlus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_gen_P08_muMinus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_gen_P09_muPlus.root",
  "/eos/user/g/gkainth/BH_filtered/filtered_gen_P09_muMinus.root"
]

tree_gen = ROOT.TChain("USR970_gen")
for f in gen_files:
  tree_gen.Add(f)
...
def reset_arrays(shape=(4,4,8)):
  ...
  rec_weights_muPlus  = np.zeros(shape, dtype=np.float64)
  gen_weights_muPlus  = np.zeros(shape, dtype=np.float64)
  acceptance_muPlus   = np.zeros(shape, dtype=np.float64)
  acceptance_err_muPlus = np.zeros(shape, dtype=np.float64)
  rec_weight2_muPlus  = np.zeros(shape, dtype=np.float64)
  gen_weight2_muPlus  = np.zeros(shape, dtype=np.float64)

  rec_weights_muMinus = np.zeros(shape, dtype=np.float64)
  gen_weights_muMinus = np.zeros(shape, dtype=np.float64)
  acceptance_muMinus  = np.zeros(shape, dtype=np.float64)
  acceptance_err_muMinus = np.zeros(shape, dtype=np.float64)
  rec_weight2_muMinus = np.zeros(shape, dtype=np.float64)
  gen_weight2_muMinus = np.zeros(shape, dtype=np.float64)

# **********************************
# Define arrays to store the acceptance/weighted sum values for each phi bin (integrate over t)
# *** mu+ ***
rec_weights_muPlus = np.zeros((9,11,8), dtype=np.float64) # holds the weighted sums for reconstructed data 
gen_weights_muPlus = np.zeros((9,11,8), dtype=np.float64) # holds the weighted sums for generated data 
acceptance_muPlus  = np.zeros((9,11,8), dtype=np.float64) # stores the acceptance values 
acceptance_err_muPlus = np.zeros((9,11,8), dtype=np.float64) # stores the estimated statistical errors 
rec_weight2_muPlus = np.zeros((9,11,8), dtype=np.float64)  # holds weight squared for each bin (reconstructed) 
gen_weight2_muPlus = np.zeros((9,11,8), dtype=np.float64)  # holds weight squared for each bin (generated)

# *** mu- ***
rec_weights_muMinus = np.zeros((9,11,8), dtype=np.float64) # array that holds the weighted sums for reconstructed data 
gen_weights_muMinus = np.zeros((9,11,8), dtype=np.float64) # array that holds the weighted sums for generated data 
acceptance_muMinus  = np.zeros((9,11,8), dtype=np.float64) # array that stores the acceptance values 
acceptance_err_muMinus = np.zeros((9,11,8), dtype=np.float64) # array that stores the estimated statistical errors
rec_weight2_muMinus = np.zeros((9,11,8), dtype=np.float64)  # holds weight squared for each bin (reconstructed) 
gen_weight2_muMinus = np.zeros((9,11,8), dtype=np.float64)  # holds weight squared for each bin (generated)
...
def fill_weights(tree, elist, charge, data_type, Q2_bins=Q2_bins, nu_bins=nu_bins, 
  ...
  for i in range(elist.GetN()):
    ...
    event = tree  # alias -> the line above pulls the exact event from the tree 

    if data_type == "gen":
      Q2 = np.float64(event.Q2_gen)
      nu = np.float64(event.nu_gen)
      t = np.float64(np.abs(event.t_gen))
      phi = np.float64(event.phi_gg_gen)
      phi = ((phi + np.pi) % (2*np.pi)) - np.pi   # wrap into [-π, π]
      weight_DVCS = np.float64(event.weight_DVCS)
    else: 
      Q2 = np.float64(event.Q2_fit)
      nu = np.float64(event.nu_fit)
      t = np.float64(np.abs(event.t_fit))
    ...
    if data_type == "rec":
      if charge == "muPlus":
        rec_weights_muPlus[i1][i2][i3] += weight_DVCS
        rec_weight2_muPlus[i1][i2][i3] += weight_DVCS ** 2
      elif charge == "muMinus":
        rec_weights_muMinus[i1][i2][i3] += weight_DVCS
        rec_weight2_muMinus[i1][i2][i3] += weight_DVCS ** 2
    elif data_type == "gen":
      if charge == "muPlus":
        gen_weights_muPlus[i1][i2][i3] += weight_DVCS
        gen_weight2_muPlus[i1][i2][i3] += weight_DVCS ** 2
      elif charge == "muMinus":
        gen_weights_muMinus[i1][i2][i3] += weight_DVCS
        gen_weight2_muMinus[i1][i2][i3] += weight_DVCS ** 2
...
def bin_in_t(tree_rec, tree_gen):
  ...
  for t_min, t_max in t_bins:
    ...
    cut_muPlus_gen = f"Q_beam > 0 && abs(t_gen) >= {t_min} && abs(t_gen) < {t_max}"
    cut_muMinus_gen = f"Q_beam < 0 && abs(t_gen) >= {t_min} && abs(t_gen) < {t_max}"
...
def plot_acceptance_by_tBin(muPlus_tBins=acceptance_muPlus_tBins, err_muPlus_tBins=acceptance_err_muPlus_tBins, 
	muMinus_tBins=acceptance_muMinus_tBins, err_muMinus_tBins=acceptance_err_muMinus_tBins, idx=0):
  ...
  for i in range(n_nu_bins):    # nu bins (y-axis)
    for j in range(n_Q2_bins):  # Q2 bins (x-axis)
      ...
      ax.errorbar(phi_bin_centers, y_muPlus, yerr=yerr_muPlus, fmt='o', markerfacecolor='none', 
                  markeredgecolor='red', markersize=7, ecolor='red', label='μ⁺' if i == 0 and j == 0 else "")
      ax.errorbar(phi_bin_centers, y_muMinus, yerr=yerr_muMinus, fmt='o', markerfacecolor='none', 
                  markeredgecolor='black', markersize=7, ecolor='black', label='μ⁻' if i == 0 and j == 0 else "")
